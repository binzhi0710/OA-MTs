...
# Read gene symbols
gene_symbols <- readLines("genes.txt")
...
# Perform pathway clustering
p <- enrichmentNetwork(all_results, colorBy = 'qvalue', colorType = 'pval', 
                       drawEllipses = TRUE, layout = "fruchterman.reingold")

# Save plot as PDF
pdf("enrichment_network.pdf", width = 30/2.54, height = 30/2.54)
print(p)
dev.off()
